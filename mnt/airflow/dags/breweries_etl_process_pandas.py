...
from config.task_config_pandas import task_dict_pandas
...
with DAG(
    "ETL-breweries-process-pandas",
    catchup=False,
    default_args=default_args,
    schedule_interval="0 0 * * *",
    max_active_runs=1,
    dagrun_timeout=timedelta(minutes=60),
    tags=["ETL", "breweries", "pandas"],
) as dag:
    
    for task_id, task_params in task_dict_pandas.items():

        py_callable = import_py_callable_and_kwargs(task_params)

        task_dict_pandas[task_id]["dag_instance"] = PythonOperator(
            task_id=task_id,
        ...
        for dependece_id in dependecies:
            (
                task_dict_pandas[task_id]["dag_instance"]
                << task_dict_pandas[dependece_id]["dag_instance"]
            )